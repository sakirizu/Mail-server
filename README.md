...
このプロジェクトでは、Gitのコミット履歴から自動的に要件定義書（`REQUIREMENTS.md`）を更新する機能があります。

### 基本的な使い方

#### 方法1: Cursor Chatから更新（推奨）

1. Cursor Chatを開く
2. 「更新」または「要件定義書を更新」と入力
3. 自動的に要件定義書が更新されます
...
- **自動変更履歴の生成**: 前回更新以降のGitコミット履歴を自動的に取得し、変更内容をMarkdown形式で整理します
- **コミット作成者の記録**: 各変更の作成者名を自動的に記録します
- **変更ファイルの一覧**: 各コミットで変更されたファイルの一覧を表示します
- **変更内容の分類**: 機能追加、バグ修正、リファクタリングなど、変更内容を自動的に分類します
- **日付ごとのグループ化**: 変更履歴を日付ごとにグループ化して表示します

### 初回更新時

初回更新時は、すべてのGitコミット履歴が要件定義書に反映されます。

### 2回目以降の更新

2回目以降は、前回更新以降の新しいコミットのみが追加されます。

### 要件定義書の内容

`REQUIREMENTS.md`には以下の情報が含まれます：
...
要件定義書はGitで管理されているため、以下のように活用できます：

1. **変更内容の共有**: チームメンバーが`git pull`で最新の要件定義書を取得できます
2. **変更履歴の追跡**: 誰が、いつ、何を変更したかが一目でわかります
3. **プロジェクトの理解**: 新しいメンバーがプロジェクトの全体像を素早く把握できます

### 注意事項

- 要件定義書の更新にはGitリポジトリが必要です
- `.requirements-last-update`ファイルは前回更新日時を記録するために使用されます（Git管理対象外）
- 手動で要件定義書を編集する場合は、変更履歴セクションを適切に更新してください
