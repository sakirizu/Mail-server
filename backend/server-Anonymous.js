...
const GlobalMailService = require('./globalMailService');
...
// Hybrid mail service (MongoDB primary, MySQL fallback)
const hybridMailService = new HybridMailService(db);
const globalMailService = new GlobalMailService();
...
async function initializeServices() {
  try {
    ...
    // Initialize hybrid mail service
    await hybridMailService.initialize();

    // Initialize global mail service
    await globalMailService.initialize(hybridMailService);
...
// Inbox endpoint: fetch emails from DB for the logged-in user
app.get('/mail/inbox', auth, async (req, res) => {
  try {
    // Example: fetch emails where recipient is the logged-in user's email
    const [rows] = await db.query(
      'SELECT id, subject, sender, snippet FROM emails WHERE recipient = ? ORDER BY id DESC',
      [req.user.email]
    );
    res.json(rows);
  } catch (e) {
...
app.post('/api/mail/send', auth, async (req, res) => {
  ...
  try {
    ...
    const result = await mailService.sendAndSaveEmail(from, to, subject, text, html);
...
// Mark email as read
app.put('/api/mail/:emailId/read', auth, async (req, res) => {
  const { emailId } = req.params;
...
// Mark email as read (hybrid system)
app.put('/api/mails/:emailId/read', auth, async (req, res) => {
  const { emailId } = req.params;
  
  try {
    const result = await hybridMailService.markAsRead(emailId, req.user.email);
    if (result.success) {
      res.json({ success: true });
    } else {
...
// Global Mail API Routes (must be before :folder route)

// Send email to any address (internal or external)
app.post('/api/mails/send-global', auth, async (req, res) => {
  try {
    ...
    console.log(`🌍 Sending global email from ${req.user.email} to ${to}: ${subject}`);
    
    const results = await globalMailService.routeEmail(
      req.user.email,
      to,
      subject,
      body || '',
    ...
    console.error('Global send email error:', error);
    res.status(500).json({ error: 'Failed to send email' });
  }
});

// Fetch emails from external accounts
app.get('/api/mails/external', auth, async (req, res) => {
  try {
    console.log(`📬 Fetching external emails for ${req.user.email}`);
    
    const externalEmails = await globalMailService.fetchExternalEmails(req.user.email);
    
    // Save external emails to our database
    for (const email of externalEmails) {
      await hybridMailService.createMail({
        from: email.from,
        to: [req.user.email],
        ...
        labels: [`external-${email.provider}`]
      });
    }
    ...
    console.error('Fetch external emails error:', error);
    res.status(500).json({ error: 'Failed to fetch external emails' });
  }
});

// Check external email providers status
app.get('/api/mails/providers', auth, async (req, res) => {
  try {
    const providers = [];
    
    if (globalMailService.smtpTransporters.has('gmail')) {
      providers.push({ name: 'Gmail', type: 'smtp', status: 'connected' });
    }
    
    if (globalMailService.smtpTransporters.has('outlook')) {
      providers.push({ name: 'Outlook', type: 'smtp', status: 'connected' });
    }
    
    if (globalMailService.smtpTransporters.has('ethereal')) {
      providers.push({ name: 'Ethereal (Test)', type: 'smtp', status: 'connected' });
    }
...
app.get('/api/mails/:folder', auth, async (req, res) => {
  try {
    ...
    const result = await hybridMailService.getMailsByUser(req.user.email, {
      folder,
  ...
  try {
    ...
    for (const recipient of [...to, ...cc, ...bcc]) {
      const recipientMailData = {
        ...mailData,
        to: [recipient],
        labels: recipient === req.user.email ? ['sent'] : ['inbox']
      };
    ...
    await mongoMailService.createMail(senderCopy);
...
app.post('/api/mails/send', auth, async (req, res) => {
  try {
    ...
    const result = await hybridMailService.createMail(mailData);
...
app.post('/api/mails/sample', auth, async (req, res) => {
  try {
    ...
    for (const email of sampleEmails) {
      const result = await hybridMailService.createMail(email);
