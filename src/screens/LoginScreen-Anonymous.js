...
export default function LoginScreen({ navigation }) {
  ...
  const handleLogin = async () => {
    if (!username || !password) {
      Alert.alert('Xatolik', 'Iltimos username va parolni kiriting');
      return;
    }
    ...
    try {
      ...
      if (data.requires2FA) {
        ...
        if (typeof window !== 'undefined') {
          window.localStorage.setItem('ssm_user_token', data.token);
        }
      } else {
  ...
  const confirmTwoFA = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch('http://10.2.145.211:4000/api/auth/signup/confirm-2fa', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
      ...
      <View style={[styles.formContainer, isMobile && styles.formContainerMobile]}>
        ...
        <View style={styles.headerContainer}>
          ...
          <Text style={[styles.subtitle, isMobile && styles.subtitleMobile]}>
            {isSignup ? 'Yangi hisob yarating' : 'Hisobingizga kiring'}
          </Text>
        </View>
        ...
        >
          <Text style={[styles.mainButtonText, isMobile && styles.mainButtonTextMobile]}>
            {loading ? 'Kutilmoqda...' : isSignup ? 'Hisob yaratish' : 'Kirish'}
          </Text>
        </TouchableOpacity>
